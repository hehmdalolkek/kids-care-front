...
export default function Edit() {
  ...
  const loadChild = async () => {
    ...
    setValue('patronymic', result.data.patronymic);
    setValue('group.id', result.data.group.id);
    setValue('dateOfBirthday', result.data.dateOfBirthday);
    setValue('mainPhone', result.data.mainPhone);
    setValue('additionalPhone', result.data.additionalPhone);
    setValue('allergies', result.data.allergies.title);
    setValue('vaccinations', result.data.vaccinations);
  };
  ...
  const onSubmit = async (data) => {
    const filteredData = allergies.filter((allergy) => allergy.title.trim() != "");
    const allergiesData = filteredData.map((allergy) => ({title: allergy.title}))
    ...
    await axios.put(`${process.env.REACT_APP_APIURL}/children/${id}`, data);
  ...
  return (
    <div className='container my-5 pt-3 rounded shadow border col-5'>
      <button className='btn' onClick={(e) => HandleGoBack(e)}>
      ...
      <h2 className='mb-5 text-center'>Изменить данные ребенка</h2>
